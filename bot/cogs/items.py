...
class Items(commands.Cog):
    ...
    async def item_new(self,ctx):
        ...
        async def do_prompt():
            await ctx.send("What would you like to name your item?(can also be an emoji)[ max_words=1 , max_len=10 ]")
            return await self.bot.wait_for("message", check=check)
        async def do_prompt_desc():
            await ctx.send("give a description of your item [ max_words=15 , max_len=150 ]")
            return await self.bot.wait_for("message", check=check)
        for _ in range(5):
            msg = await do_prompt()
    ...
    async def item_restock(self,ctx):
        ...
        await self.bot.db.item.update_one({"_id":ctx.author.id},{"$inc":{"max_stock":max_inc}})
        await self.bot.db.item.update_one({"_id":ctx.author.id},{"$set":{"stock":stock_inc}})
        await ctx.send(f"restocked {stock_inc}items and max stock increased by {max_inc}")
