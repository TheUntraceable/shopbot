...
class buy_menu_wall(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        profile = await self._view.bot.db.economy.find_one({"_id": self._view.ctx.author.id})
        if profile.get("wallet") < 100:
            await interaction.response.edit_message(
                content="You don't have enough money"
            )
            return
        await self._view.bot.db.economy.update_one({"_id": self._view.ctx.author.id}, {"$inc": {"wallet": -100}})
        doc = await self._view.bot.db.up.find_one({"_id": self._view.ctx.author.id})
        doc["wall"].append(self.values[0])
        await self._view.bot.db.up.update_one({"_id": self._view.ctx.author.id}, {"$set": {"wall": doc["wall"]}})
